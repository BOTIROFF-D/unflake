...
**No partial-order reduction here.** [pnueli](https://github.com/BOTIROFF-D/pnueli) is where that got built — ample sets, symmetry reduction, and a measured 625× on a specification this could only enumerate flat. In this package, `explore` enumerates systematically but does not *reduce*: unflake cannot see which operations touch shared state, so it cannot prove two orderings equivalent and skip one. The tree grows multiplicatively and exhaustive coverage stays out of reach for anything but small tests. Rust's `loom` does reduce, because code under test uses loom's own atomics and mutexes and it therefore sees every shared access. Matching that in JavaScript would mean asking users to declare their shared resources — a real design cost, and the main open question for this project.
